...
if($_POST)
{
    ...
    if(!isset($_POST["name"]) || !isset($_POST["number"]) || !isset($_POST["street"]) || !isset($_POST["postal"]) || !isset($_POST["majint"]) || !isset($_POST["city"]))
    {
    ...
    $alt          = filter_var($_POST["check3"], FILTER_SANITIZE_STRING);
    $additional   = filter_var($_POST["check2"], FILTER_SANITIZE_STRING);
	...
	if ($additional=='true'){
		...
		{
			$output = json_encode(array('type'=>'error', 'text' => ''.$additional.'Comment is too short, ensure proper formatting!'));
			die($output);
		}
	}
	...
	$qry = "INSERT INTO location(code, login, name, phone, alternate, type, type_number, street, postal, city, intersection, comments, is_type, is_alternate, is_additional, added_date) 
	VALUES('$code','$login','$name','$number','$alternate','$type','$tnum','$street','$postal','$city','$major','$comments','$subunits','$alt','$additional','$date')";
	//Check whether the query was successful or not
	$result = @mysqli_query($GLOBALS["___mysqli_ston"], $qry);
    if(!$result)
    {
        $output = json_encode(array('type'=>'error', 'text' => "Could not add location! Please contact an IT Specialist."));
        die($output);
    }else{
        $output = json_encode(array('type'=>'message', 'text' => 'Hi '.$login.', the following location has been added: <b>'.$name.'</b>, click the away if you are finished adding locations.'));
        die($output);
    }
} else {
