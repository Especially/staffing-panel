...
// $collate = filter_var($_GET["collate"], FILTER_SANITIZE_STRING); -> If collate = true select * from shifts where `name` LIKE '%Location_name%'
...
<p class="ss_date"><?php echo ($start_expand.'-'.$end_expand) ?></p></center>
    ...
    <table class="flatTable" data-sort="<?php echo($sort_type); ?>">
...
echo('
		<td class="controlTd ss_controltd">
			...
			<td class=\"total_td\">".$hours." hours</td>
...
$qry3 = "SELECT * FROM shifts WHERE filled='1' AND cancelled='0' AND location='$location' AND date < '$start_date' ORDER BY date DESC, IN1 ASC LIMIT 50";
$res3 = mysqli_query($GLOBALS["___mysqli_ston"], $qry3);
echo "    $('#appendit').html(\"<option value='null' selected='selected' disabled>Append A Shift...</option>";
if(mysqli_num_rows($res3)>0){
	while($row3 = mysqli_fetch_assoc($res3)){
        $filled_by = $row3['ename'];
		...
		$time_12_hour_OUT  = date("g:i A", strtotime("$hour2:$minute2"));
        $code = $row3['code'];
        echo "<option value='$code'>$date $time_12_hour_IN - $time_12_hour_OUT $filled_by</option>";
    }
}
echo "\");
    var appnd = $(\"#appendit\").html();
    var is_filled = $(\"#v_cal .flatTable\").data('sort');
    if (!appnd || appnd == '' || is_filled !== 'Filled'){
        $(\"#info_append\").slideUp();
        $(\".append_span input[type='button']\").prop('disabled',true);
    } else {
        $(\"#info_append\").slideDown();
        $(\".append_span input[type='button']\").prop('disabled',false);
    }
    $(\".append\").off('click');
    $(\".append\").on('click', function(){
        var shift_id = $(\"#appendit\").val();
        if (shift_id !== 'null'){
        $(this).prop('disabled',true);
        $(\"#appendit option[value='\"+ shift_id +\"']\").remove();
        $(\"#appendit option[value='null']\").prop('selected',true);
        var hours = parseFloat($(\".total_td\").text(), 10);
        console.log(shift_id);
            $.ajax({
                type: \"POST\",
                dataType: \"json\",
                url: root+\"go_fetch.php\", 
                data: {'action':'shift_data','code':shift_id},
                success: function(data) {
                    $(\"#v_cal .flatTable tr:last\").before(\"<tr class=\"+ data.type +\"><td>\"+ data.date +\"</td><td>\"+ data.filled_by +\"</td><td>\"+ data.time_12_hour_IN +\"</td><td>\"+ data.time_12_hour_OUT +\"</td><td class='controlTd ss_controltd'>&nbsp;</td></tr>\");
                    console.log(JSON.stringify(data));
                    var new_hrs = hours + parseFloat(data.hours, 10);
                    $(\".total_td\").text(new_hrs + \" hours\");
