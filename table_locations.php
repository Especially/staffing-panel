...
text-shadow: 0px 1px 0px rgba(255, 255, 255, 0.5);">Work Sites</h2></center>
...
while($row = mysqli_fetch_assoc($result)){
	...
	echo(
	'<tr class="'.$row['code'].'">
	<td><a href="/Locations/List/Calendar/'.$row['code'].'" class="pll-t" data-calendar-title="Shifts at '.$row['name'].'" onclick="viewCal(&quot;'.$row['code'].'&quot;)">'.$row['name'].'</a></td>
	...
	<td class="controlTd controlEL">
		<div class="settingsIcons">
			...
			<div class="settingsIcon" onclick="editPE(&quot;'.$row['code'].'&quot;,&quot;location&quot;)">Edit</div>
		</div> 
	</td>
	</tr>');
	}
	...
	echo(	
	'<tr class="'.$row['code'].'">
	<td><a href="/Locations/List/Calendar/'.$row['code'].'" class="pll-t" data-calendar-title="Shifts at '.$row['name'].' ('.$row['type'].' '.$row['type_number'].')" onclick="viewCal(&quot;'.$row['code'].'&quot;)">'.$row['name'].' ('.$row['type'].' #'.$row['type_number'].')</a></td>
	<td><a href="tel:+'.$phone.'">'.$row['phone'].'</a></td>
	<td><a href="tel:+'.$alt.'">'.$row['alternate'].'</a></td>
	<td>'.$row['street'].'</td>
	<td>'.$row['intersection'].'</td>
