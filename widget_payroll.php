...
$pr_from = filter_var($_GET["pr_from"], FILTER_SANITIZE_STRING);
$pr_to = filter_var($_GET["pr_to"], FILTER_SANITIZE_STRING);
$employ = "SELECT * FROM employee WHERE euid='$id'";
$employ = "SELECT DISTINCT euid FROM shifts WHERE filled='1' AND cancelled='0' AND date >= '$d1' and date <= '$d2'";
...
<div id="holder" class="mCustomScrollbar light holder" data-mcs-theme="light-thick">
	...
	<div id="box" data-location="Employee Profile">
		...
		$nextor = "SELECT DISTINCT @row:=@row+1 as rank, a.euid,a.ename FROM shifts a WHERE date >= '$d1' and date <= '$d2', (SELECT @row:=0) b ORDER BY a.sname ASC";
...
$(document).ready( function(){
    ...
    $(\"#nextor\").change(function(){
        var sel = $(this).val();
        $('#widget_ep').load(''+root+'employee_profile?id='+sel+'', function() {
...
";
		 ...
		 ?>
        	...
        	<div id="ep_info">
                ...
                <span class="address"><?php echo("$street<br>$city, ON Canada"); ?></span>
                <span class="comments"><div class="sprite_calc" title="Calculate the sum of hours between certain dates for this employee"></div><p style="margin:0px;"><?php if ($hours > 0){echo("$hours hours successfully completed.");} else{ echo('No hours completed yet'); } ?></p><?php echo("$additional"); ?></span>
            </div>
		...
		<tr class='headingTr'>
			...
			");
				$qry = "SELECT * FROM shifts WHERE euid='$id' AND filled='1' AND cancelled='0' ORDER BY date LIMIT 25";
