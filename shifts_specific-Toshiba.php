...
while($row1 = mysqli_fetch_assoc($sol)){
    ...
    $collatable = "SELECT COUNT(name) AS collates FROM location WHERE name = '$name'";
    $collrate = mysqli_query($GLOBALS["___mysqli_ston"], $collatable);
    while($coll = mysqli_fetch_assoc($collrate)){
        $collcount = $coll['collates'];
        if ($collcount > 1){
            $can_collate = true;
        } else {
            $can_collate = false;
    ...
    $type_numb = $row1['type_number'];
    $street = $row1['street'];
    $postal = $row1['postal'];
    $city = $row1['city'];
    $inter = $row1['intersection'];
    if ($type_numb == 0 || $type_numb == '') {
        $street = $street;
    } else {
        $street = $type_numb."-".$street;
    }
    $address = $street."<br/>".$city.", ON<br/>".$postal;
}
?>
<html>
<head>
...
<div id="info_box">
    <h1><i class="fa fa-info-circle"></i><?php echo($name); ?></h1>
    <div id="info_content">
        <div id="info_phones">
            <div class="info-line">
                <span style="text-align:right;"><i class="fa fa-phone"></i></span>
        ...
        <div id="info_address" class="info-line">
            <span style="text-align:right;"><i class="fa fa-map-marker"></i></span>
            <span><?php echo($address); ?></span>
        </div>
        <div id="info_append" class="info-line">
            <span style="text-align:right;"><i class="fa fa-plus-square"></i></span>
            <span class="append_span"><select id="appendit"></select><input type="button" class="blue append" value="Append" style="width:75px;" disabled/></span>
        </div>
    </div>
